...
void ProcessFile(const char* filename, const std::string& originalHistNameTru, const std::string& originalHistNameUnf, const char* canvasName,bool base) {
    int pt1 = 70;
    int pt2 = 120 - 1;
    ...
    TH1D* h1;
    if(base){
        h1 = h2->ProjectionX("h1", h2->GetYaxis()->FindBin(pt1), h2->GetYaxis()->FindBin(pt2));
        h1->Scale(1., "width");
        ...
        h1 = h2->ProjectionY("h1", h2->GetXaxis()->FindBin(pt1),h2->GetXaxis()->FindBin(pt2));
        h1->Scale(1., "width");
        h1->SetLineColor(kRed);
        h1->SetMarkerStyle(20);
        h1->SetMarkerColor(kRed);
...
//        "~/Desktop/pbpbAnalysis5Tev/Data18q.root"
    ...
    std::string histNamesUnf[4] = {"Opt_Un_e3c", "Opt_Un_eec","h3_JMB1MB2_dat","h3_JMBMB_dat"};
    std::string histNamesTru[4] = {"E3C_pt_hist", "EEC_pt_hist", "h_JMB1MB2_dat","h_JMBMB_dat"};
    
    bool base = true;
    //    ProcessFile(filenames[0], histNamesTru[0], histNamesUnf[0], "Canvas_E3C");
    ProcessFile(filenames[1], histNamesTru[0], histNamesUnf[0], "Canvas_EEC",base);
