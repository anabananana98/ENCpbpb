...
void getcfactorsEEC() {
    std::string filename = "~/Desktop/pbpbAnalysis5Tev/embForCfactorEEC.root";
    std::string filename_cfac = "~/Desktop/pbpbAnalysis5Tev/embForCfactorEEC.root";
    // Open the ROOT file
    ...
    if(ifCorrectHistograms){
        ...
        if(if1D){
            ...
            // Create a canvas
            TCanvas* ctest = new TCanvas("ctest", "Histogram Comparison", 800, 600);
            ctest->cd();
            gPad->SetLogx();
            h1_MB1_m->Draw();
            h1_MJ0_m->Draw("same,HIST");
            
            TCanvas* cnewt = new TCanvas("cnewt", "Histogram Comparison", 800, 600);
            cnewt->cd();
            gPad->SetLogx();
            ////############################//#################################//#################################
            ...
            std::string fileOutName = "cfactorEEC.root";
            TFile* fileOut = new TFile(fileOutName.c_str(), "UPDATE");
            h1_MB1MB2_corr_m->Write(Form("c_fac_mb1mb2_m_%i_%i",pt1,pt2));
            h1_MB1MB2_corr_um->Write(Form("c_fac_mb1mb2_um_%i_%i",pt1,pt2));
            h1_MB1_corr_m->Write(Form("c_fac_mb1_m_%i_%i",pt1,pt2));
            h1_MB1_corr_um->Write(Form("c_fac_mb1_um_%i_%i",pt1,pt2));
            h1_SMB_corr->Write(Form("c_fac_smb_%i_%i",pt1,pt2));
            fileOut->Write();  // Ensure all objects are written
            fileOut->Close();  // Properly close the file
            delete fileOut;    // Avoid memory leak
...
//            h3_M_tot_corrected_3D->Add(h3_M0_tot_corrected_3D_um);
            ...
            TH3D* h3_M1_MJ_3D =  (TH3D*)f3->Get("h3Jet_deltaR_MJ1_m");//only matched sb contribution
            ...
            h3_JMB_3D->Add(h3_BMB_um_3D);//jmb=((s+b)_matched + b_unmatched)*mb
...
//            TH3D* h3_JMB_3D_clone = (TH3D*) h3_JMB_3D->Clone("h3_JMB_clone_3D");
            ...
            if(tag=="cFactor"){
                ...
                cfac_h3_JMB->Divide(h3_M1_MJ_3D); //THIS IS HOW YOU DO THE CORRECTION
                
                
                TH3D* cfac_h3_SMB = (TH3D*)h3_JMB_3D->Clone("cfac_h3_SMB");
                //                TH3D* h3_JMB_corrected_3D_samefile = (TH3D*)h3_JMB_3D->Clone("h1_JMB_corrected");
                //                cfac_h3_JMB->Add(h3_MB1MB2_corrected_3D_samefile,-1);
                cfac_h3_SMB->Divide(h3_M1_MJ_3D);
                ...
                //#######Write to output file##################//
                std::string fileOutName = "cfactorEEC_3D.root";
                TFile* fileOut = new TFile(fileOutName.c_str(), "UPDATE");
                cfac_h3_MB1MB2_m->Write(Form("c_fac_mb1mb2_m_%i_%i",pt1,pt2));
                cfac_h3_MB1MB2_um->Write(Form("c_fac_mb1mb2_um_%i_%i",pt1,pt2));
                cfac_h3_MB1_m->Write(Form("c_fac_mb1_m_%i_%i",pt1,pt2));
                cfac_h3_MB1_um->Write(Form("c_fac_mb1_um_%i_%i",pt1,pt2));
                cfac_h3_SMB->Write(Form("c_fac_smb_%i_%i",pt1,pt2));
                fileOut->Write();  // Ensure all objects are written
                fileOut->Close();  // Properly close the file
                delete fileOut;    // Avoid memory leak
